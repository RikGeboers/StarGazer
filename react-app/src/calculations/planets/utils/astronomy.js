...
var EARTH_RADII_PER_ASTRONOMICAL_UNIT =
...
function DefaultGeocentricCoordinates(day) {
  // this function is used for most celestial bodies (but not the Moon)
...
function EarthClass() {
  ...
  this.EclipticCartesianCoordinates = function (day) {
    ...
    var M0 =
      357.5291 + 35999.0503 * T - 0.0001559 * T * T - 0.00000048 * T * T * T; // Sun's mean anomaly, in degrees

    var C = // Sun's equation of center in degrees
      (1.9146 - 0.004817 * T - 0.000014 * T * T) * Angle.SinDeg(M0) +
      (0.01993 - 0.000101 * T) * Angle.SinDeg(2 * M0) +
      0.00029 * Angle.SinDeg(3 * M0);
    ...
    var distanceInAU =
      (1.000001018 * (1 - e * e)) / (1 + e * Angle.CosDeg(M0 + C)); // distance from Sun to Earth in astronomical units (AU)
...
function PlutoClass() {
  ...
  this.EclipticCartesianCoordinates = function (day) {
    ...
    var lonecl =
      238.9508 +
      0.00400703 * day -
      19.799 * Angle.SinDeg(P) +
      19.848 * Angle.CosDeg(P) +
      0.897 * Angle.SinDeg(2 * P) -
      4.956 * Angle.CosDeg(2 * P) +
      0.61 * Angle.SinDeg(3 * P) +
      1.211 * Angle.CosDeg(3 * P) -
      0.341 * Angle.SinDeg(4 * P) -
      0.19 * Angle.CosDeg(4 * P) +
      0.128 * Angle.SinDeg(5 * P) -
      0.034 * Angle.CosDeg(5 * P) -
      0.038 * Angle.SinDeg(6 * P) +
      0.031 * Angle.CosDeg(6 * P) +
      0.02 * Angle.SinDeg(S - P) -
      0.01 * Angle.CosDeg(S - P);
    var latecl =
      -3.9082 -
      5.453 * Angle.SinDeg(P) -
      14.975 * Angle.CosDeg(P) +
      3.527 * Angle.SinDeg(2 * P) +
      1.673 * Angle.CosDeg(2 * P) -
      1.051 * Angle.SinDeg(3 * P) +
      0.328 * Angle.CosDeg(3 * P) +
      0.179 * Angle.SinDeg(4 * P) -
      0.292 * Angle.CosDeg(4 * P) +
      0.019 * Angle.SinDeg(5 * P) +
      0.1 * Angle.CosDeg(5 * P) -
      0.031 * Angle.SinDeg(6 * P) -
      0.026 * Angle.CosDeg(6 * P) +
      0.011 * Angle.CosDeg(S - P);
    var r =
      40.72 +
      6.68 * Angle.SinDeg(P) +
      6.9 * Angle.CosDeg(P) -
      1.18 * Angle.SinDeg(2 * P) -
      0.03 * Angle.CosDeg(2 * P) +
      0.15 * Angle.SinDeg(3 * P) -
      0.14 * Angle.CosDeg(3 * P);
...
function PlanetPS( // See  http://www.stjarnhimlen.se/comp/ppcomp.html#4
  ...
  N0,
  Nc, // N0 = longitude of the ascending node (deg).  Nc = rate of change in deg/day
  i0,
  ic, // inclination to the ecliptic (deg)
  w0,
  wc, // argument of perihelion (deg)
  a0,
  ac, // semi-major axis, or mean distance from Sun (AU)
  e0,
  ec, // eccentricity (0=circle, 0..1=ellipse, 1=parabola)
  M0,
  Mc, // M0 = mean anomaly  (deg) (0 at perihelion; increases uniformly with time).  Mc ("mean motion") = rate of change in deg/day = 360/period
  magBase,
  magPhaseFactor,
  magNonlinearFactor,
...
PlanetPS.prototype.EclipticAngularCoordinates =
  DefaultEclipticAngularCoordinates;

PlanetPS.prototype.MeanAnomaly = function (
  day // day = number of days elapsed since December 31, 1999 0:00 UTC, i.e. JD 2451543.5
) {
...
PlanetPS.prototype.DistanceFromEarth = function (
  day // returns the distance of this planet from Earth in astronomical units (AU)
) {
  return this.GeocentricCoordinates(day).Distance();
};
...
PlanetPS.prototype.VisualMagnitude = function (day) {
  ...
  var mag =
    this.magBase + 5 * Log10(distSun * distEarth) + this.magPhaseFactor * phase;
...
function EccentricAnomaly(e, M) {
  ...
  for (;;) {
    var F =
      E -
      (E - Angle.DEG_FROM_RAD * e * Angle.SinDeg(E) - M) /
        (1 - e * Angle.CosDeg(E));
...
function EqCoords(day, location) {
  ...
  var K =
    23.4392911 - (46.815 * T - 0.00059 * T * T + 0.001813 * T * T * T) / 3600.0; // obliquity of ecliptic in degrees.
  ...
  // RE/r >= pi / (180 * 3600)
  var parallaxInRadians =
    1.0 / (EARTH_RADII_PER_ASTRONOMICAL_UNIT * distanceInAU);
  if (parallaxInRadians >= Math.PI / (180.0 * 3600.0)) {
    ...
    var altitude =
      hor.altitude -
      Angle.DEG_FROM_RAD * parallaxInRadians * Angle.CosDeg(hor.altitude);
    ...
    var g =
      Angle.DEG_FROM_RAD *
      Math.atan(Angle.TanDeg(gclat) / Angle.CosHour(hourAngle));

    var topRA =
      eq.longitude -
      (parallaxInRadians *
        Angle.HOURS_FROM_RAD *
        rho *
        Angle.CosDeg(gclat) *
        Angle.SinHour(hourAngle)) /
        Angle.CosDeg(eq.latitude);
    var topDEC;
    ...
    if (Math.abs(g) < 1.0e-6) {
      topDEC =
        ...
        parallaxInRadians *
          Angle.DEG_FROM_RAD *
          rho *
          Angle.SinDeg(-eq.latitude) *
          Angle.CosHour(hourAngle);
    } else {
      topDEC =
        eq.latitude -
        (parallaxInRadians *
...
function HorizontalCoordinates(
  ...
  if (absAltitudeRatio > 1.0) {
    ...
    if (
      absAltitude < ANGLE_CORRECTION_WINDOW &&
      horizonCorrectionInArcMinutes != 0.0
    ) {
      ...
      var linearFactor =
        (ANGLE_CORRECTION_WINDOW - absAltitude) / ANGLE_CORRECTION_WINDOW;
      this.altitude -= (horizonCorrectionInArcMinutes / 60.0) * linearFactor;
    }

    this.azimuth = Angle.FixDegrees(
      Angle.AtanDeg2(
        -cosDec * sinHourAngle,
        cosLat * sinDec - sinLat * cosDec * cosHourAngle
      )
    );
  }
}
...
function CreatePlanetJPL( //  Designed for use with JPL orbital data
  name, // name of the object, e.g. "Mars"
  ja0,
  jac, // semi-major axis (AU), rate (AU/century)
  je0,
  jec, // eccentricity (1), (1/century)
  jI0,
  jIc, // inclination (deg), (deg/century)
  jL0,
  jLc, // mean longitude (deg), (deg/century)
  ju0,
  juc, // longitude of perihelion (deg), (deg/century)
  jQ0,
  jQc, // longitude of ascending node (deg), (deg/century)
  magBase,
  magPhaseFactor,
  magNonlinearFactor,
  magNonlinearExponent
...
function CreateJupiter() {
  var planet = CreatePlanetJPL(
    "Jupiter",
    5.202887,
    -0.00011607, // ja0, jac = semi-major axis (AU), rate (AU/century)
    0.04838624,
    -0.00013253, // je0, jec = eccentricity (1), (1/century)
    1.30439695,
    0.00183714, // jI0, jIc = inclination (deg), (deg/century)
    34.39644051,
    3034.74612775, // jL0, jLc = mean longitude (deg), (deg/century)
    14.72847983,
    0.21252668, // ju0, juc = longitude of perihelion (deg), (deg/century)
    100.47390909,
    0.20469106, // jQ0, jQc = longitude of ascending node (deg), (deg/century)
    -9.25,
    0.014,
    0,
    0 // visual magnitude elements)
  );
...
function CreateSaturnJPL() {
  var planet = CreatePlanetJPL(
    "SaturnJPL",
    9.53667594,
    -0.0012506, // ja0, jac = semi-major axis (AU), rate (AU/century)
    0.05386179,
    -0.00050991, // je0, jec = eccentricity (1), (1/century)
    2.48599187,
    0.00193609, // jI0, jIc = inclination (deg), (deg/century)
    49.95424423,
    1222.49362201, // jL0, jLc = mean longitude (deg), (deg/century)
    92.59887831,
    -0.41897216, // ju0, juc = longitude of perihelion (deg), (deg/century)
    113.66242448,
    -0.28867794, // jQ0, jQc = longitude of ascending node (deg), (deg/century)
    -9.0,
    0.044,
    0,
    0 // visual magnitude elements
  ...
  planet.VisualMagnitude = function (day) {
    ...
    var Las = Angle.FixDegrees(
      Angle.AtanDeg2(gc.z, Math.sqrt(gc.x * gc.x + gc.y * gc.y))
    );
...
function CreateUranus() {
  var planet = CreatePlanetJPL(
    "Uranus",
    19.18916464,
    -0.00196176, // ja0, jac = semi-major axis (AU), rate (AU/century)
    0.04725744,
    -0.00004397, // je0, jec = eccentricity (1), (1/century)
    0.77263783,
    -0.00242939, // jI0, jIc = inclination (deg), (deg/century)
    313.23810451,
    428.48202785, // jL0, jLc = mean longitude (deg), (deg/century)
    170.9542763,
    0.40805281, // ju0, juc = longitude of perihelion (deg), (deg/century)
    74.01692503,
    0.04240589, // jQ0, jQc = longitude of ascending node (deg), (deg/century)
    -7.15,
    0.001,
    0,
    0 // visual magnitude elements
...
function CreateMoon() {
  var moon = new PlanetPS(
    ...
    318.0634,
    0.1643573223,
    60.2666 / EARTH_RADII_PER_ASTRONOMICAL_UNIT,
    0,
    0.0549,
    0,
    115.3654,
  ...
  moon.Perturb = function (xh, yh, zh, d) {
    ...
    var deltaRadius =
      -0.58 * Angle.CosDeg(Mm - 2 * D) - 0.46 * Angle.CosDeg(2 * D);
    var ecliptic = Astronomy.EclipticLatLon(xh, yh, zh);
    var lonecl = ecliptic.longitude;
    var latecl = ecliptic.latitude;
...
function AstronomyClass() {
  this.DayValue = function (utc) {
    ...
    return (
      1.0 + (utc.getTime() - Date.UTC(2000, 0, 1)) / (3600.0 * 24.0 * 1000.0)
    );
  };
  ...
  this.NextRiseTime = function (body, day, location) {
    return Astronomy_FindNextTransition(
      body,
      day,
      day + 1.1,
      location,
      Astronomy_RiseCondition,
      3
    );
  };
  ...
  this.NextNewMoon = function (day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_RelativeLongitudeCondition,
      numIntervals,
      0.0
    );
  };
  ...
  this.NextMaxSunAngle = function (body, day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_MaxSunAngleCondition,
      numIntervals
    );
  };
  ...
  this.NextVernalEquinox = function (day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_VernalEquinoxCondition,
      numIntervals
    );
  };
  ...
  this.NextAutumnalEquinox = function (day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_AutumnalEquinoxCondition,
      numIntervals
    );
  };
  ...
  this.NextNorthernSolstice = function (day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_NorthernSolsticeCondition,
      numIntervals
    );
  };
  ...
  this.NextSouthernSolstice = function (day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_SouthernSolsticeCondition,
      numIntervals
    );
  };
  ...
  this.NextPeakVisualMagnitude = function (body, day, numDays, numIntervals) {
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_PeakVisualMagnitudeCondition,
      numIntervals
    );
  };

  this.NextMinAngleWithOtherBody = function (
    body,
    day,
    numDays,
    numIntervals,
    ...
    return Astronomy_FindNextTransition(
      ...
      day + numDays,
      null,
      Astronomy_MinAngleWithOtherBodyCondition,
      numIntervals,
      otherBody
    );
  };
  ...
  //
  this.ConstellationRotationMatrix = this.PrecessionRotationMatrix(
  ...
  this.PrecessEquatorialCoordinates = function (eq, matrix) {
    ...
    var x1 = [A * Math.cos(RA1), A * Math.sin(RA1), Math.sin(DEC1)];
  ...
  this.FindConstellation = function (eq) {
    var eqOld = this.PrecessEquatorialCoordinates(
      eq,
      this.ConstellationRotationMatrix
    );
  ...
  this.Ida = CreateMinor(
    ...
    1768.623391686735,
    9.94
  ); // http://ssd.jpl.nasa.gov/sbdb.cgi?sstr=243
  this.Gaspra = CreateMinor(
    "Gaspra",
    2457000.5,
    253.1678768421052,
    4.102608826356864,
    ...
    1199.908645279267,
    11.46
  ); // http://ssd.jpl.nasa.gov/sbdb.cgi?sstr=951
  this.Comet_9P = CreateComet(
    "9P/T1",
    2456717.5,
    68.88181348869867,
    10.50258430337173,
    ...
    2032.415859334781,
    13.1
  ); // comet 9P/Tempel 1 : http://ssd.jpl.nasa.gov/sbdb.cgi?sstr=9P
  this.Comet_19P = CreateComet(
    "19P/B",
    2456870.5,
    75.3828362947892,
    30.36856213315691,
    ...
    2497.570436475743,
    8.9
  ); // comet 19P/Borrelly : http://ssd.jpl.nasa.gov/sbdb.cgi?sstr=19P
  this.Comet_67P = CreateComet(
    "67P/C-G",
    2456981.5,
    50.14210951437195,
    7.040200902346087,
    ...
    2353.654952366357,
    13.55
  ); // comet 67P/Churyumov-Gerasimenko : http://ssd.jpl.nasa.gov/sbdb.cgi?sstr=67P
  this.Comet_81P = CreateComet(
    "81P/W2",
    2456978.5,
    136.1250968664513,
    3.239043597517584,
...
function Astronomy_CulminateCondition(body, day1, day2, location) {
  ...
  // The altitude at rboth times must be above the horizon (altitude > 0).
  if (hc1.altitude > 0) {
...
function Astronomy_MinAngleWithOtherBodyCondition(
  body,
  day1,
  day2,
  location,
...
function Astronomy_RelativeLongitudeCondition(
  body,
  day1,
  day2,
  location,
...
function Astronomy_FindNextTransition(
  body,
  startDay,
  endDay,
  location,
  ...
  for (var i = 0; i < numIntervals; ++i) {
    ...
    if (condition(body, t1, t2, location, customData)) {
      if (dt < 1.0e-6) {
        ...
        var evt = Astronomy_FindNextTransition(
          body,
          t1,
          t2,
          location,
...
function GeographicCoordinates(
  longitude,
  latitude,
  elevationInMetersAboveSeaLevel
) {
  ...
  this.latitude = latitude;
  this.radius =
    elevationInMetersAboveSeaLevel / METERS_PER_ASTRONOMICAL_UNIT +
